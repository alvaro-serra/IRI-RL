...
"""
Basic classes to create Agents with 
basic saving and loading functions.
...
class AgentsFactory:
	...
	@staticmethod
	def get_by_name(agent_name, *args, **kwargs):
		if agent_name == 'DQN':
			from .dqn import DQN
			agent = DQN(*args, **kwargs)
		else:
			raise ValueError("Model %s not recognized." % agent_name)
...
class BaseAgent(object):
	...
	def __init__(self,opt):
		...
		# Question: need to ask what is this about (to train with multiple gpus?)
		self._master_gpu_id = opt["agent"]["master_gpu"]
		self._reg_gpus_ids = opt["agent"]["reg_gpus"]

		self._device_master = f"cuda:{self._master_gpu_id}" if torch.cuda.is_available() else "cpu"
		self._reg_device_master = f"cuda:{self._reg_gpus_ids}" if torch.cuda.is_available() else "cpu"
	...
	def _load_optimizer(self, optimizer, optimizer_laber, epoch_label):
	...
	### This module loads the network in different forms but I don't get the subtilities behind the code reasoning
